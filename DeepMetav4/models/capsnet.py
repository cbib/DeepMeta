...
The current version maybe only works for TensorFlow backend.
Actually it will be straightforward to re-write to TF code.
...
def CapsNet(input_shape, n_class, routings):
    ...
    :return: Two Keras Models, the first one used for training,
    ...
    primarycaps = uc.PrimaryCap(
        conv1, dim_capsule=8, n_channels=32, kernel_size=9, strides=2, padding="valid"
    )
    # output here is (None, None, 8)
    # print("prim cap", np.shape(primarycaps))
    # Layer 3: Capsule layer. Routing algorithm works here.
    digitcaps = uc.CapsuleLayer(
    ...
    out_caps = uc.Length(name="capsnet")(digitcaps)
    ...
    masked_noised_y = uc.Mask()([noised_digitcaps, y])
...
def margin_loss(y_true, y_pred):
    ...
    """
    L = y_true * K.square(K.maximum(0.0, 0.9 - y_pred)) + 0.5 * (
        1 - y_true
    ) * K.square(  # noqa
        K.maximum(0.0, y_pred - 0.1)
    )
...
def test(model, data, args):
    ...
    img = uc.combine_images(np.concatenate([x_test[:50], x_recon[:50]]))
    image = img * 255
...
def manipulate_latent(model, data, args):  # a quoi sers tu ?
    ...
    img = uc.combine_images(x_recons, height=16)
    image = img * 255
    Image.fromarray(image.astype(np.uint8)).save(
        args.save_dir + "/manipulate-%d.png" % args.digit
    )
    print(
        "manipulated result saved to %s/manipulate-%d.png"
        % (args.save_dir, args.digit)  # noqa
    )
...
if __name__ == "__main__":
    ...
    # used in routing algorithm. should > 0")
    ...
    model, eval_model, manipulate_model = CapsNet(
        input_shape=(28, 28, 1), n_class=2, routings=2
    )
    model.summary()
    ...
    #         print('No weights are provided. Will test using
